...
請分析這張貨架圖片，並偵測是否含有「Stone Bay Sauvignon Blanc 2021」此產品。  
嚴格遵守以下規範，僅回傳最終 JSON 結果，**不做其他敘述**。

1. 分析規則  
   - 只偵測「Stone Bay Sauvignon Blanc 2021」，忽略其他品牌與包裝。  
   - 若未偵測到，則 `"found": false` 且 `"count": 0`，`"locations": []`。  
   - 若偵測到，一定要回傳每個包裝的 bounding box 座標（左上角 x1,y1，右下角 x2,y2）。

2. 產品參考資訊  
     "廠名": "佰事達物流股份有限公司",
      "品名": "Stone Bay Sauvignon Blanc 2021",
      "規格": "",
      "條碼": "",
      "包裝特色": [
        "綠色玻璃瓶身",
...
```json
{
  "product": "Stone Bay Sauvignon Blanc 2021",
  "found": <true|false>,
  "locations": [
...
# 測試圖片資料夾 & 標籤資料夾
TEST_FOLDER = "/Users/y/Documents/Programming/大四上/價格辨識/中文辨識/新品/紐西蘭石頭灣白酒"
...
for filename in sampled:
    ...
    for loc in locations:
        # 檢查是否有完整的 x1, y1, x2, y2
        if not all(k in loc for k in ("x1", "y1", "x2", "y2")):
            print(f"[WARNING] {filename} 跳過不完整座標：{loc}")
            continue
